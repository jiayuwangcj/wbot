...
**serve 代理**（产品组体验意见 7，2026-08-02 起）：浏览器不能直连网关（loopback，CORS/安全），`wbot serve` 代为访问富途网关（复用本客户端，限频池内置；消费方：浏览器 Dashboard 账户/订单 + CLI/脚本）。四个只读代理：

| 端点 | 行为 | 网关地址 |
| --- | --- | --- |
| `GET /v1/futu/quote?symbol=HK.00700` | 实时报价——代订阅+取快照并透传 s2c；消费方为 CLI/脚本（前端报价卡已随 Dashboard 改造移除，2026-08-02） | REST `FUTU_GATEWAY_URL`（默认 `http://127.0.0.1:22222`） |
...
`option-quote`（combo）实测：body `{"multi_legs":[{"security":{"market":1,"code":"..."},"side":1,"qty_ratio":1}]}`，s2c `option_quote_list[]` 含 `price/mid/iv/delta/gamma/vega/theta/rho/open_interest/vol/strike/days_to_expiry/contract_size`；**一次仅一个合约**（多腿=组合报价非批量），快照限频 1 次/3s，**免订阅**——`iv` 为百分数（实测 HK 盘后 TCH260814C375000：price 106.79、iv 122.07、delta 0.986、theta -0.247），无盘口（mid 是唯一参考价）。

**实时报价 adapter（2026-08-11 实测修正）**：`internal/futu.OptionQuotes`（wheel 运行器数据面）双层拉取——① 快照层 `POST /api/subscribe`（`sub_types=[1]`）后 `POST /api/quote` 批量（`security_list` 多合约单次），解析 `s2c.basic_qot_list[].cur_price/volume/update_time`（**无 bid/ask，`option_ex_data` 恒 null**，sub_types=[1,7] 不改变字段）；② Greeks 层对缺失/过期合约逐合约 `POST /api/option-quote`（3s 限频），填 Delta/IV（百分数 ÷100 归一为小数）/Theta/OI/Bid/Ask——Bid=Ask=mid（mid≤0 时回退 price），Last 取快照 cur_price（0 时回退 option-quote price）；包级缓存 10min 内不重拉（首轮 85 合约≈255s≈一个 5min tick，之后每两 tick 补一轮，快照每 tick 全量）。字段缺失留零值，运行器走 [[WHEEL_STRATEGY]] DATA_BLOCKED 兜底。
