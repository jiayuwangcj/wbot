...
- **peak 时段**：主 agent / PM 组做**轻 token 工作**（取任务、分诊、计划、状态跟踪、文档、巡检）；**避免重 token 工作**（大段编码、长多轮评审、批量实现）——顺延 off-peak 或拆小
- **off-peak 时段**：安排重工作（编码、完整评审、批量化任务）
- PM 组拆出 **dispatcher**（短期任务调度）与 **efficiency**（工程运行提效）两个独立 agent（均为轻 token 职责），主会话/PM 组自行评估工作分配
