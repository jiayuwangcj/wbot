...
- 战略层由人定义价格—目标库存曲线、最大库存和状态，不由回测优化。曲线第一点（最低价）= 满仓 `max_inventory`，最后一点（最高价）= 清仓 0，区间内持仓分配由策略线性插值自主决定（2026-08-13）。
- 合约乘数（lot size）不接受配置：运行时从行情 `contract_size` 实时拉取，拉不到按 100 兜底；`lot_size` 旧配置键静默忽略（2026-08-13）。
