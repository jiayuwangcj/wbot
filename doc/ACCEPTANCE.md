...
| 逐端点 e2e | `scripts/accept-*.sh`（20 个，238 项） | 各子系统 CLI/HTTP 真实契约（含真实网关/真实 PG） | 每个闭环提交前，连跑两遍；**零依赖对与 PG 依赖对已在 CI 自动跑**（#52/#53/#56/#57） |
...
| `accept-hkex-datafill.sh` | HKEX DTOP/RP006 日终字段、双表幂等、Wheel 完整周期/成交与研究态非零收益 | 10 | 本地确定性假 HKEX + 真实 PG。`scripts/accept-hkex-datafill.sh [bin] [dsn]` | ✅ db-integration |
...
| ingest | accept-bars-refill / accept-tencent-datafill / accept-hkex-datafill / accept-options-ingest / accept-account-snapshot | bars/futu-option/account 含 HTTP；Tencent/HKEX 为专用 CLI | 含 `-every` 循环、Tencent 一次性回填与 HKEX 双表幂等研究投影 |
| futu | accept-futu-cli 21 | accept-futu-data 15 | order 只测 -dry-run 与校验/红线拒绝路径，**绝不下真单**（写操作 + 账户状态变更，刻意无自动脚本） |
| backtest | accept-backtest 21 + report 14 + push 13 + train 8 | 同上（GET detail/export） | 四条字节一致等价 + from_watchlist + 版本化报告/Discord 幂等推送/ES 实测 |
...
本次对账实计：20 个 `accept-*.sh`，检查项为 `7+15+11+13+14+21+8+4+5+21+15+10+6+2+4+12+8+16+22+24=238`；其中各脚本检查数由 `grep -c 'check "'` 实计。
