...
| 层 | 入口 | 覆盖 | 何时跑 |
| --- | --- | --- | --- |
| 单测 + 静态 | `scripts/verify.sh`（= CI `test` job） | 全部 Go 包单测、gofmt、契约测试 | 每次提交前 |
...
| 逐端点 e2e | `scripts/accept-*.sh`（12 个，122 项） | 各子系统 CLI/HTTP 真实契约（含真实网关/真实 PG） | 每个闭环提交前，连跑两遍 |
...
| 脚本 | 覆盖 | 检查数 | 前置 / 运行 |
| --- | --- | --- | --- |
| `accept-agent-federation.sh` | master/agent 联邦（register/agents/错误契约 + `wbot agent` e2e 自注册） | 11 | 无 PG 无网关。`scripts/accept-agent-federation.sh` |
| `accept-paper.sh` | `wbot paper` CLI 契约（side 别名/非法 side/空 symbol） | 12 | 纯本地。`scripts/accept-paper.sh` |
...
| `accept-backtest.sh` | backtest 双面（CLI -dsn/-save/-export + GET detail/export；四条字节一致等价） | 19 | serve + PG + 种子 bars。`scripts/accept-backtest.sh [base-url] [bin] [dsn] [symbol]` |
| `accept-watchlist.sh` | `wbot watchlist` CLI（add/remove/list + 写面→读面联动） | 14 | serve + PG。`scripts/accept-watchlist.sh [bin] [dsn] [base-url]` |
...
| backtest | accept-backtest 19 | 同上（GET detail/export） | 四条字节一致等价实测 |
| paper | accept-paper 12 | — | 纯本地无网络 |
| watchlist | accept-watchlist 14 | dev-up（PUT/GET /v1/watchlist）+ 联动断言 | 独立 symbol 不留痕 |
