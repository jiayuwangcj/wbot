...
## 780 竞态第三次实锤(#63 部署前最后一次,2026-08-14 02:00 北京)

- **780 链**:17:51:37 创建 → 17:56:37 gate 第一次失败(LLM_REVIEW_FAILED)→ ~17:56:57 推送器双通道 skip+推进游标 → 重试 17:56:40 开始 → **18:00:26 重试成功落 REJECTED(审核裁决拒绝下单)** → 780 已 skip,审核成功被吞,卡片不推
- **第三次实证**(764/772/780 同型):一次 gate 超时 = 信号必丢,重试承诺失效;#63 修复(窗口内保持游标)部署后此场景应变为:FAILED 窗口内 retry → 重试成功落 REJECTED → 推送器下 tick 读 LatestLLMReview 推 REJECTED 卡
- 780 裁决为 REJECTED(拒绝卡),损失为「老板未收到不操作通知」;若为 APPROVE(下单卡)则丢的是下单机会——资金安全关键路径
- **部署紧迫**:#63 已合入(7febed3),serve 部署前每遇 gate 失败即丢卡;780 为部署前最后实证,部署后验收:gate 失败 → 推送器打「window open, will re-check」而非 skip,重试成功卡片送达

## ⚠️ 新观测:775 评估循环卡死 19 分钟(2026-08-14 01:45 北京)

- **775 时间线**:17:26:20 wheel_signals 行写入(ALERT)→ 推送器 17:28 起双通道 retry(正常,游标保持)→ **wheelrun ALERT 评估日志 17:45 才出现(间隔 ~19 分钟)** → gate 调用此时才开始。774 对照:17:17:39 创建 → 17:21:05 LLM_REVIEW(评估+gate 共 ~3.5 分钟)
- **结论**:wheelrun 评估循环 17:26→17:45 之间静默卡死 ~19 分钟(可能行情拉取/DB/网关阻塞),自愈后继续。与 770 推送循环 3.5 分钟静默(2026-08-14 16:59:27→17:02:57)同类——**评估循环无心跳/无超时日志,卡死不可观测**
- 775 gate 进行中(17:45+),审核结果待观察;若成功 → 推送验证;若失败 → serve 当前镜像(15d0a9a,无 #63)会 skip,竞态第三次实证
- **排期建议**:wheelrun 评估循环补心跳/阶段超时日志(#62 推送循环心跳是同类修复,评估循环缺失)。评估延迟容忍:正常 ~3.5 分钟,19 分钟不可接受(期间新信号堆积)

## 789 正常对照(2026-08-14 02:4x 北京,serve 部署后最后观察)

- **789 链**:18:36:45 信号创建(ALERT)→ 18:39:01 REJECTED 落库(gate 一次通过,无 LLM_REVIEW_FAILED,评估 2.2 分钟正常,无 775 型卡死)→ telegram 推 REJECTED 卡 + discord 处理 → 双循环 heartbeat cursor=789 pending=false 收口
- serve(15d0a9a 镜像)下 774-789 连续闭环全部正常,唯一例外 780(gate 失败被吞,#63 部署前最后实证);#63(7febed3)合入主基线待部署
