...
<body><main class="shell">
  <header><div class="eyebrow">schema {{.Report.SchemaVersion}} · {{.Report.ReportID}}</div><h1>{{.Report.Identity.Symbol}} 回测报告</h1><div class="status">{{.Report.Identity.CapabilityStatus}}</div></header>
    <section class="grid" aria-label="核心指标">
      ...
      <article class="card"><div class="label">窗口收益（研究值）</div><div class="value">{{.WindowMark}}</div><div class="sub">{{.WindowAmount}}</div></article>
      <article class="card"><div class="label">年化收益</div><div class="value">{{percentPtr .Report.Result.AnnualizedReturnPct}}</div><div class="sub">自然日窗口折算</div></article>
      <article class="card"><div class="label">窗口末估值变动（研究值）</div><div class="value">{{.WindowMark}}</div><div class="sub">{{.WindowAmount}} · {{.Report.Result.ReturnStatus}}</div></article>
