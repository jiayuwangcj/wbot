...
<body><main class="shell">
  <header><div class="eyebrow">schema {{.Report.SchemaVersion}} · {{.Report.ReportID}}</div><h1>{{.Report.Identity.Symbol}} 回测报告</h1><div class="status">{{.Report.Identity.CapabilityStatus}}</div></header>
    <section class="grid" aria-label="核心指标">
      ...
      <article class="card"><div class="label">{{if .Report.Result.PremiumNetReturnPct}}权利金净收益{{else}}已实现盈利{{end}}</div><div class="value">{{percentPtr .Report.Result.NetReturnPct}}</div><div class="sub">{{.Report.Result.ReturnStatus}}{{if .Report.Result.PremiumNetReturnPct}} · 权利金 − 平仓 − 期权/行权费用{{else}} · 权利金 − 平仓 − 正股已实现 − 费用{{end}}</div></article>
      {{if .Report.Result.PremiumNetReturnPct}}<article class="card"><div class="label">权利金净收益金额</div><div class="value">{{amountPtr .Report.Result.PremiumNetReturnAmount .Report.Identity.Currency}}</div><div class="sub">premium_net_amount,评价口径</div></article>{{end}}
      <article class="card"><div class="label">已实现年化</div><div class="value">{{percentPtr .Report.Result.AnnualizedReturnPct}}</div><div class="sub">自然日窗口折算</div></article>
