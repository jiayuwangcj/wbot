...
func TestPositionsInput(t *testing.T) {
	...
	stock, opts, err := PositionsInput(context.Background(), pos)
	if err != nil {
...
func TestPositionsInputSymbolFallback(t *testing.T) {
	...
	stock, opts, err := PositionsInput(context.Background(), []Position{
...
func TestPositionsInputErrors(t *testing.T) {
	if _, _, err := PositionsInput(context.Background(), []Position{{Code: "", Symbol: ""}}); err == nil {
		t.Fatal("PositionsInput(empty code) = nil error; want error")
	}
	if _, _, err := PositionsInput(context.Background(), []Position{{Symbol: "HK.00700", Code: "00700", Qty: 1, Side: 9}}); err == nil {
