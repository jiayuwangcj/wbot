...
var agentTools = []map[string]any{
	...
	toolDefinition("option_chain", "查询期权链(合约、行权价、到期日、权利金、delta/iv、持仓量;min_dte/max_dte 按剩余天数过滤,不传则不过滤)", map[string]any{"symbol": stringProperty("正股代码"), "min_dte": integerProperty("最小到期天数"), "max_dte": integerProperty("最大到期天数"), "max_strikes": integerProperty("最多行权价数")}, []string{"symbol"}),
	toolDefinition("option_quote", "查询指定期权合约报价(delta/iv/open_interest/权利金)", map[string]any{"contract": stringProperty("期权合约代码")}, []string{"contract"}),
	toolDefinition("account", "查询账户资金及指定标的正股持仓数量、期权 Delta 汇总", map[string]any{"symbol": stringProperty("正股代码")}, []string{"symbol"}),
}
...
func (c *Client) Generate(ctx context.Context, s Snapshot) (llmsignal.Decision, error) {
	...
	for round := 0; round < c.maxRounds; round++ {
		...
		for _, call := range message.ToolCalls {
			...
			fmt.Fprintf(os.Stderr, "llmstrategy: %s: agent tool call: %s(%s) round=%d\n", s.Symbol, call.Function.Name, call.Function.Arguments, round+1)
...
func (snapshotToolExecutor) OptionChain(_ context.Context, symbol string, minDTE, maxDTE int, maxStrikes int, s Snapshot) (any, error) {
	if symbol != s.Symbol {
		return nil, fmt.Errorf("symbol %q is outside the current snapshot", symbol)
	}
	opts := filterByDTE(s.Options, minDTE, maxDTE, time.Now())
...
// filterByDTE keeps only options whose expiry falls inside [minDTE, maxDTE]
// trading days from now; expired or unparseable contracts never reach the
// model. Zero bounds leave the corresponding side open.
func filterByDTE(opts []Option, minDTE, maxDTE int, now time.Time) []Option {
	if minDTE <= 0 && maxDTE <= 0 {
		return opts
	}
	filtered := make([]Option, 0, len(opts))
	for _, o := range opts {
		t, err := time.Parse(time.RFC3339, o.Expiry)
		if err != nil || !t.After(now) {
			continue
		}
		dte := int(t.Sub(now).Hours()/24) + 1
