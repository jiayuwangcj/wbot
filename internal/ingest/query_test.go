...
func TestQueryBars_validation(t *testing.T) {
	...
	if _, err := QueryBars(ctx, nil, "X.US", "1d", "none", time.Time{}, time.Time{}, 10); err == nil {
		t.Fatal("expected error for nil db")
	}
	if _, err := QueryBars(ctx, stubDB(), "", "1d", "none", time.Time{}, time.Time{}, 10); err == nil {
		t.Fatal("expected error for empty symbol")
	}
	if _, err := QueryBars(ctx, stubDB(), "X.US", "", "none", time.Time{}, time.Time{}, 10); err == nil {
		t.Fatal("expected error for empty timeframe")
	}
	if _, err := QueryBars(ctx, stubDB(), "X.US", "1d", "none", from, to, 10); err == nil {
	...
	for _, limit := range []int{0, -1} {
		if _, err := QueryBars(ctx, stubDB(), "X.US", "1d", "none", time.Time{}, time.Time{}, limit); err == nil {
