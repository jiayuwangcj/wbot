...
func TestThemeSystem(t *testing.T) {
	...
	for _, want := range []string{`localStorage.getItem("wbot-theme")`, "dataset.theme", "initTheme();", "redrawCharts()", "chartCache", `cssVar("--surface") || "#fff"`} {
		if !strings.Contains(string(js), want) {
...
func TestInteractionFeedbackJS(t *testing.T) {
	...
	for _, want := range []string{
		...
		"hideOk()",
		`"admin-refresh"`,
		"wrapRefreshClick(\"admin-refresh\"",
