...
## 成本时段(降本增效,2026-07-31 指令;2026-08-11 补充)
...
- **codex CLI 不受高峰约束(2026-08-11 用户指令)**:codex 走 ChatGPT 订阅额度(非按量 token 计费),高峰时段仍可照常派 codex 编码(遵守 codex 单飞);成本时段约束只作用于 Claude 侧模型。
...
- **并发(2026-08-11 用户指令)**:codex 本身保持单飞(同时最多一个 `codex exec`,并发保护互相误判);**Claude 侧 coder/subagent 可与进行中的 codex 任务并发**(不同 worktree、文件不重叠、合入串行)。
