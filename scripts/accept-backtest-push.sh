...
embed_ok="$(node -e 'const q=require("fs").readFileSync(process.argv[1],"utf8").trim().split("\n").map(JSON.parse)[1].payload;const f=q.embeds[0].fields.map(x=>x.name);const want=["标的","数据窗口","净收益 / 能力状态","有效覆盖率","费用","最大回撤","停止原因"];process.stdout.write(String(f.length===7&&want.every(x=>f.includes(x))));' "$tmp/requests.ndjson")"
check "Discord embed 含 7 个核心字段" true "$embed_ok"
