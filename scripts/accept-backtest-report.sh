...
summary_ok="$(node -e 'const r=require(process.argv[1]).result,o=process.argv[2];const amount=Math.abs(r.net_return_amount-r.net_return_pct*10000)<1e-9;const att=require(process.argv[1]).result.attribution;const pn=att.premium_net_amount;const pnOk=Math.abs(pn-(att.premium_income_amount-att.option_close_cost_amount-(require(process.argv[1]).result.cost_model.option_fees_amount+require(process.argv[1]).result.cost_model.exercise_delivery_fees_amount)))<1e-9;const line=o.includes(`premium_net_return=${pn/10000}`)&&o.includes(`mark_return=${r.window_mark_to_market_return_pct}`)&&o.includes(`max_drawdown=${r.max_drawdown_pct}`);process.stdout.write(String(amount&&pnOk&&line));' "$json_path" "$out1")"
