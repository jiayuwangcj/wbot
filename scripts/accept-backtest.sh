...
check "CLI summary 形状 final_equity/realized_return/premium_net_return/mark_return/max_drawdown/bars" \
  1 "$(echo "$out" | grep -cE '^final_equity=.+ realized_return=.+ premium_net_return=.+ mark_return=.+ max_drawdown=.+ bars=[0-9]+( 未成交.*)?$')"
