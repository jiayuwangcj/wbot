...
#
# Usage: scripts/accept-option-freshness.sh
# Requires: go, docker (for wbot-pg-ci-test address discovery).
...
DSN="postgres://postgres:postgres@$(docker inspect wbot-pg-ci-test --format '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}'):5432/wbot_test?sslmode=disable"
builddir="$(mktemp -d)"
...
out="$(WBOT_PG_DSN="$DSN" "$WB" ingest freshness 2>&1)"
code=$?
...
out2="$(WBOT_PG_DSN="$DSN" "$WB" ingest freshness -max-age 1000000h 2>&1)"
code2=$?
...
docker exec -i wbot-pg-ci-test psql -U postgres -d wbot_test -c "DELETE FROM option_quotes WHERE underlying IN ('ACCOPTFRESH.US','ACCOPTSTALE.US')" >/dev/null 2>&1
